...
import { getTeacherRoster, saveTeacherAttendance } from '../../services/attendanceService.js'
...
export default function AttendancePage() {
  ...
  const classes = [...new Map((assignments ?? []).map((a) => [a.class.id, a.class])).values()]
  ...
  const { data, loading, error } = useAsync(
    () => (classId ? getTeacherRoster(classId, date) : Promise.resolve(null)),
    [classId, date],
  )
  ...
  const handleSave = async (records) => {
    ...
    setSaveError(null)
    try {
      await saveTeacherAttendance(classId, { date, records })
      setSaved(true)
    } catch (err) {
  ...
  return (
    <PageContainer>
      <div className="space-y-6">
        <PageHeader title="Attendance" description="Take daily attendance for your classes." />

        <Card>
          <CardHeader title="Select class & date" description="Pick one of the classes you teach" />
